...
`;
      ...
      if (response.functionCalls && response.functionCalls.length > 0) {
        ...
        setMessages(prev => [...prev, { id: Date.now() + 1, sender: 'ai', text: response.text ?? 'Xin lỗi, em không thể tạo phản hồi.' }]);
      }

    } catch (error) {
      console.error("AI Assistant Error:", error);
      setMessages(prev => [...prev, { id: Date.now() + 1, sender: 'ai', text: 'Xin lỗi, em không có quyền truy cập hoặc đã xảy ra lỗi.' }]);
    } finally {
      setIsLoading(false);
    }
  };
